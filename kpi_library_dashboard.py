...
KPI library categorization structure
...
Instructions
# Ditteau Data — KPI Library & Example Dashboard Project Context > Portable context carried over from the main Ditteau Data project. > Scope of the new project: build a **KPI Library** (centralized, governed definitions of the catalog KPIs) and an **example dashboard** on top of the existing marts. > Maintained by: Serensoft Data Engineering (LVP) --- ## 1. What Ditteau Data Is The **Ditteau Data Unified Platform** is Serensoft's multi-tenant higher-education analytics product — "enterprise intelligence at institutional scale" for small-to-midsize colleges. It ingests legacy SIS and adjacent-system data into a governed analytics layer on Snowflake, exposes a star-schema Distribute layer, and delivers KPIs/dashboards via Streamlit in Snowflake. For this project, the platform is the **backend**; the deliverables are the **KPI definition library** and a **reference dashboard** that consumes existing marts. --- ## 2. Team & Governance Gates | Handle | Role | Gate | |--------|------|------| | **LVP** | Architecture & Strategic Planning | — | | **KKM** | Data Governance & Quality | Sign-off required for FERPA-sensitive content, PII/`data_class` changes, client-facing material | | **WDT** | Systems & Security / Infra | Required reviewer for all production merges; owns Snowflake provisioning; executes Claude Code instructions | | **RDT** | Strategic Placement & Marketing | Peer/competitive content | Refer to members by handle. Two gates matter most for a dashboard build: **KKM before any FERPA-flagged KPI reaches client-facing output**, and **WDT for any production merge or role/warehouse provisioning**. --- ## 3. Client Schools & Source Systems | School | Primary SIS | Status | |--------|-------------|--------| | Merrimack College | Jenzabar CX → Jenzabar One (migrating) | Reference implementation | | Saint Anselm College | JCX (legacy) + Workday Student | Active | | Colby College | JCX | Active | | Springfield College | Ellucian Banner | Onboarding | | Endicott College | JCX (`.4gl` variant) | Onboarding | Source systems feeding the KPIs: **Jenzabar CX / Jenzabar One, Workday Student, Ellucian Banner** (the three interchangeable SIS options, written `Jenzabar:Workday:Banner` in the catalog), plus **Slate** (admissions CRM), **PowerFAIDS** (financial aid), and external feeds **NSC / IPEDS**. --- ## 4. Architecture (backend the dashboard sits on) Three-layer medallion on Snowflake: - **Deposit (Bronze)** — raw landing; **not dbt-managed** (Org Share for CX; `deposit_loader.py` for CSV). School isolation at database level. - **Deterge (Silver)** — dbt; `stg_` (views, source conformance) and `int_` (tables, cross-source logic). `int_student_versions` is the UNION ALL router with `has_*` var guards; `dim_integration_ids` resolves cross-system IDs. - **Distribute (Gold)** — dbt; star schema (`dim_`, `fact_`), SCD Type 2 on `dim_student`, masking + row-access policies applied **here only**. Marts live under `distribute/marts/`. Multi-tenancy: `var('school_code')` routing; `<school>__<model>.sql` overrides with `alias` config; `safe_col` for absent columns; `school_ref()` in override-bearing intermediates. --- ## 5. KPI Catalog — the centerpiece Source of truth: `higher_ed_kpi_catalog.csv` — **⚠️ row count to reconcile:** the catalog CSV was last counted at 127 entries (96 KPIs + 31 dashboards); `docs/kpi_catalog/kpi_library.html` was generated with 138 rows. Confirm the authoritative count directly from the CSV before publishing either artifact. **By Area** (from 127-row CSV snapshot — recount if CSV was updated) | Area | Entries | |------|---------| | Enrollment Management | 32 | | Admissions | 30 | | Financial Aid | 29 | | Registration | 27 | | Cross-Domain | 9 | **By Type:** Strategic 59 · Operational 27 · Compliance 21 · Financial 20 **Time series:** 53 yes / 74 no · **Indicator:** Lagging 41 · Leading 12 · N/A 74 **FERPA sensitive:** 30 yes / 97 no (the 30 are the gate for KKM review before client-facing use) **Update cadence:** mostly `Term` (54) and `Annual` (49), with `Weekly` admissions/registration items and a few `Daily (spring)`. Every catalog row is assigned to one of **19 marts/views**. This mapping is the primary contract between the catalog and the dashboard. --- ## 6. Marts / Views Backing the Catalog Ranked by number of catalog entries served: | Mart / View | Entries | Domain | Build Status | |-------------|---------|--------|--------------| | `snap_cohort_milestone` | 16 | Cross-domain / retention | ⚠️ **Unbuilt — critical path** (see note below) | | `mart_admissions_funnel` | 13 | Admissions | Built | | `mart_section_utilization` | 13 | Registration | Built | | `mart_aid_summary` | 13 | Financial Aid | Built | | `snap_admissions_weekly` | 12 | Admissions | Built | | `mart_enrollment_census` | 9 | Enrollment | Built | | `snap_aid_term` | 9 | Financial Aid | Built | | `snap_enrollment_term` | 7 | Enrollment | Built | | `mart_admissions_class_profile` | 5 | Admissions | Built | | `mart_retention_cohort_summary` | 5 | Enrollment | Built | | `mart_enrollment_census_ntr` | 4 | Enrollment | Status unknown | | `mart_ipeds_reporting` | 4 | Compliance | Pending NSC/IPEDS feeds | | `mart_academic_progress` | 3 | Registration | Built | | `mart_aid_leveraging` | 3 | Financial Aid | Built | | `snap_retention_term` | 3 | Enrollment | Built | | `mart_executive_summary` | 3 | Cross-domain | Status unknown | | `mart_registration_holds` | 2 | Registration | Built | | `mart_student_at_risk` | 2 | Registration | Built | | `mart_ipeds_fall_enrollment` | 1 | Compliance | Pending NSC/IPEDS feeds | | `mart_enrollment_demographics` | — | Cross-domain | ⚠️ **Build status unknown** (see note below) | **Build-status notes:** - **`snap_cohort_milestone` (unbuilt):** Backs all 16 Cross-Domain catalog entries and is the sole dependency blocking the Cross-Domain dashboard section. `kpi_library_dashboard.py` already queries it on the Cross-Domain tab — the tab will throw a Snowflake "object does not exist" error until the mart is built. Treat as the critical path for Cross-Domain coverage. - **`mart_enrollment_demographics`:** Queried by `kpi_library_dashboard.py` on the Cross-Domain tab (URM%, first-gen%, demographic breakdowns) but absent from the catalog mart list above. Build status is unknown; if unbuilt, it is a second unresolved dependency on the Cross-Domain page. - **`mart_enrollment_census` vs. `mart_enrollment_census_ntr`:** Naming alignment is unresolved — both appear as catalog targets. Pin this before wiring enrollment tiles, as the NTR variant may serve a distinct grain (e.g., non-traditional students). - `mart_ipeds_reporting` / `mart_ipeds_fall_enrollment` activate when NSC + IPEDS feeds are provisioned. --- ## 7. Conventions That Matter for KPI/Dashboard Work - **Rates are decimals, not percentages** (e.g., `0.62`, not `62`). Format at the presentation layer. - **`snake_case` columns**; `COALESCE` for NULL-safe aggregations. - **`dim_student` is SCD Type 2** — never filter `is_current = true` except for explicit current-state tiles. Point-in-time KPIs need full history. - **`fact_enrollment` range joins anchor to `first_event_date`**, not `begin_date`. - **`mart_student_at_risk` uses `is_at_risk` (boolean), not `is_current`** — the mart has no `is_current` column. Filter with `WHERE is_at_risk = TRUE`. Also exposes `risk_tier` ('high'/'medium'/'none') and `risk_factor_count`. - **Marts consume from `dim_`/`fact_` only** (`ref()`, never `source()`); materialized as tables (pre-computed for BI). - **No masking logic in SQL** — masking is enforced by Snowflake policies on the Distribute schema. - **Snowflake GROUP BY quirk:** Cannot reference column aliases in GROUP BY — must repeat the full expression (including full CASE expressions). - Use **exact catalog names** for KPIs, marts, and view targets when referencing them. - **Centralizing calculation logic is the main gap a KPI library fills** — the catalog + dbt conventions already supply most of what a semantic layer needs. `docs/kpi_catalog/` is the home for non-buildable reference material; `models/` stays reserved for actual dbt assets. Don't adopt semantic-layer tooling speculatively before first go-live. --- ## 8. Validation Benchmarks (KPI ↔ UAT contract) `V01_V05_validation_benchmarks.xlsx` is the IR-completed benchmark set that validates mart outputs in Phase 4 UAT. All five tabs must be filled before UAT begins. These define the "official numbers" each KPI family is checked against: | Artifact | Validates | KPIs (examples) | |----------|-----------|-----------------| | V-01 Headcount & FTE | `mart_enrollment_census` | Headcount vs. Budget, FTE, FT/PT split, New vs. Continuing, Credits per FTE | | V-02 Graduation & Completions | `is_graduated` flag + grad-rate KPIs | 4yr/6yr grad rate, cohort survival curves, revenue per completed student | | V-03 Admissions Funnel | `mart_admissions_funnel`, `snap_admissions_weekly` | App-to-admit, admit-to-yield, deposit conversions, yield trend | | V-04 Financial Aid Totals | FA marts | Total aid, need vs. merit, loan vs. grant, Pell recipients, inst. aid % of tuition | | V-05 Race & Ethnicity | race/ethnicity pipeline, URM | URM % (Hispanic+Black+AIAN+NHPI+2ormore), first-gen %, stop-out by demographic | When defining a KPI in the library, link it to its validating benchmark tab where one exists — that's the acceptance criterion. --- ## 9. Delivery Mechanism Dashboards ship as **Streamlit in Snowflake (Snowpark)**, scoped to the `DISTRIBUTE` schema, with per-school `[CODE]_REPORTING_PROD` roles and `[CODE]_ANALYTICS_PROD` warehouses. The example dashboard should assume this target and read from marts in Distribute, not from Deterge or Deposit.
...
higher_ed_kpi_catalog_enriched.csv
csv
...
with st.sidebar:
    ...
    area = st.radio(
        ...
        ["Admissions", "Registration", "Financial Aid",
         "Enrollment Management", "Cross-Domain"],
